...
public class FalconPathPlanner {
     ...
     * {15,6}, {15, 4} }; This path goes from {1,1} -> {5,1} -> {9,12} -> {12, 9} ->
     * {15,6} -> {15,4} The units of these coordinates are position units assumed by
     * the user (i.e inch, foot, meters)
     *
     * @param path
     */
    ...
    public static void test(double[][] waypoints) {
        long start = System.currentTimeMillis();

        double totalTime = 8;
        double timeStep = 0.01;
        double robotTrackWidth = RobotParameters.WHEEL_BASE;
     ...
     * @param robotTrackWidth - width between left and right wheels of robot of skid
     *                        steer chassis.
     */
    public void leftRight(double[][] smoothPath, double robotTrackWidth) {
        ...
        for (int i = 0; i < gradient.length; i++) {
            ...
            if (i > 0) {
                ...
                } else if ((deg - gradient[i - 1][1]) < -180) {
                    gradient[i][1] = 360 + deg;
                }
            }
     ...
     *
     * @param totalTime       - time the user wishes to complete the path in
     *                        seconds. (this is the maximum amount of time the robot
     *                        is allowed to take to traverse the path.)
     * @param timeStep        - the frequency at which the robot controller is
     *                        running on the robot.
     * @param robotTrackWidth - distance between left and right side wheels of a
     *                        skid steer chassis. Known as the track width.
     */
